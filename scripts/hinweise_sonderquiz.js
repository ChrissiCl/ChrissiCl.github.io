var meineDaten = 
{
	'quize': [
		{
			'name': '1. Advent Quiz',
			'abschlussseite': 'raetselende_ersteradvent.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen ersten Advent',
				'kategorie': 'Neue Schwierigkeitsstufe - wird nicht verraten 😉',
				'hinweise': ['Ich wurde als einziger aus meiner Familie domestiziert', 'Ich mag es gerne kalt', 'Ich wohne im Norden', 'Ich esse gerne Flechten', 'Ich unternehme weite Wanderungen', 'Ich bin ein Sternbild', 'In Europa gibt es nur noch wenige wilde Exemplare von mir'],
				'loesung': ['Rentier']
			}
			]
		},
		{
			'name': '2. Advent Quiz',
			'abschlussseite': 'raetselende_zweiteradvent.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen zweiten Advent',
				'kategorie': 'Wird immer noch nicht verraten 😉',
				'hinweise': ['Es gibt mich 4 Mal', '1926 wurde ich überflogen', '2007 wurde ich mit einem Auto erreicht', 'Ich liege im kleinsten Ozean der Welt', 'Das Meer ist bei mir 4087m tief', 'Ich wandere', 'Mein politischer Status ist unklar'],
				'loesung': ['Nordpol']
			},
			{
				'raetselname': 'Noch ein Rätsel',
				'kategorie': 'Was wohl?',
				'hinweise': ['Ich bin ein Gegenstand von Spekulationen', 'Ich bestehe aus Gas', 'Es gibt sehr viele von mir', 'Ich durchlaufe verschiedene Lebensphasen', 'Man kann mich lesen', 'Ich habe Ecken und Kanten oder bin eine Kugel', 'Zwerg oder Riese'],
				'loesung': ['Stern']
			}
			]
		},
		{
			'name': '3. Advent Quiz',
			'abschlussseite': 'raetselende_dritteradvent.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen dritten Advent',
				'kategorie': 'Gute Frage 😉',
				'hinweise': ['Ich bin ein Bühnenstück', 'Ich bin ein Werkzeug', 'Es gibt mich in verschiedenen Varianten', 'Ich bin eine Holzfigur', 'Ich kann funktional oder dekorativ sein', 'Ich komme in einem Märchen vor', 'Ist mein Gegner eine Nuss oder eine Maus?'],
				'loesung': ['Nussknacker']
			},
			{
				'raetselname': '2. Rätsel',
				'kategorie': '?',
				'hinweise': ['Ich bin offen oder geschlossen', 'Ich bin ein enger Felsspalt', 'Mit Pulver kann man durch mich reisen', 'Ich habe eigenes Besteck', 'Ich bin entspannend', 'Heutzutage bin ich vor allem dekorativ', 'Ich habe ein Sims'],
				'loesung': ['Kamin']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': '?',
				'hinweise': ['Ich diene dem Transport', 'Ich bin ein Musikant', 'Ich kann Gold spucken', 'Ich bin eigensinnig', 'Ich gelte als nicht besonders schlau', 'Mit einer Brücke bin ich hilfreich', 'Mein Name ist "Equus asinus"'],
				'loesung': ['Esel']
			}
			]
		},
		{
			'name': '3. Advent Quiz',
			'abschlussseite': 'raetselende_vierteradvent.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen vierten Advent',
				'kategorie': 'Mal sehen 😉',
				'hinweise': ['Ich bin struppig', 'Man findet mich im Stall', 'Ich bin trocken', 'Niemand will wie ich sein', 'Ich bin ein Überbleibsel', 'Manche Sterne bestehen aus mir', 'Ich bin blond'],
				'loesung': ['Stroh']
			},
			{
				'raetselname': '2. Rätsel',
				'kategorie': '?',
				'hinweise': ['Ich bin dunkel', 'Ich bin ein Violinkonzert von Vivaldi', 'Ich kann tief sein', 'In der Meteorologie beginne ich früher', 'Mein Name ist ein germanisches Sonderwort', 'Ich lasse vieles schlafen', 'Manchmal bin ich hart'],
				'loesung': ['Winter']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': '?',
				'hinweise': ['Ich bin eine Institution', 'Ich bin diplomatisch', 'Ich kann gut oder schlecht sein', 'Ich kann aus Papier sein', 'Meine Überbringer können vielgestaltig sein', '1974 ging ich an Außerirdische', 'Man findet mich in Hauptstädten'],
				'loesung': ['Botschaft']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': '?',
				'hinweise': ['Ich bin klein', 'Tomte Tummetott', 'Ich lebe versteckt', 'Ich tue Gutes', 'In Skandinavien helfe ich dem Weihnachtsmann', 'Die Maskottchen des ZDF gehören zu meiner Art', 'Ich bin ein Diminutiv'],
				'loesung': ['Wichtel']
			}
			]
		},
		{
			'name': '1. Advent Quiz',
			'abschlussseite': 'raetselende_ersteradvent2.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen ersten Advent',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin eine geometrische Form', 'Ich bin eine runde Sache', 'Ich bin ein Geschoss', 'Mit Rum bin ich eine Süßigkeit', 'Ich beschäftige Mathematiker', 'Ich bin ein Symbol für Unendlichkeit', 'Ich gelte als vollkommen'],
				'loesung': ['Kugel']
			}
			]
		},
		{
			'name': '2. Advent Quiz',
			'abschlussseite': 'raetselende_zweiteradvent2.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen zweiten Advent',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Es gibt mich 4 Mal', '1926 wurde ich überflogen', '2007 wurde ich mit einem Auto erreicht', 'Ich liege im kleinsten Ozean der Welt', 'Das Meer ist bei mir 4087m tief', 'Ich wandere', 'Mein politischer Status ist unklar'],
				'loesung': ['Nordpol']
			},
			{
				'raetselname': 'Noch ein Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin ein Gegenstand von Spekulationen', 'Ich bestehe aus Gas', 'Es gibt sehr viele von mir', 'Ich durchlaufe verschiedene Lebensphasen', 'Man kann mich lesen', 'Ich habe Ecken und Kanten oder bin eine Kugel', 'Zwerg oder Riese'],
				'loesung': ['Stern']
			}
			]
		},
		{
			'name': '3. Advent Quiz',
			'abschlussseite': 'raetselende_dritteradvent2.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen dritten Advent',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin ein Bühnenstück', 'Ich bin ein Werkzeug', 'Es gibt mich in verschiedenen Varianten', 'Ich bin eine Holzfigur', 'Ich kann funktional oder dekorativ sein', 'Ich komme in einem Märchen vor', 'Ist mein Gegner eine Nuss oder eine Maus?'],
				'loesung': ['Nussknacker']
			},
			{
				'raetselname': '2. Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin offen oder geschlossen', 'Ich bin ein enger Felsspalt', 'Mit Pulver kann man durch mich reisen', 'Ich habe eigenes Besteck', 'Ich bin entspannend', 'Heutzutage bin ich vor allem dekorativ', 'Ich habe ein Sims'],
				'loesung': ['Kamin']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich diene dem Transport', 'Ich bin ein Musikant', 'Ich kann Gold spucken', 'Ich bin eigensinnig', 'Ich gelte als nicht besonders schlau', 'Mit einer Brücke bin ich hilfreich', 'Mein Name ist "Equus asinus"'],
				'loesung': ['Esel']
			}
			]
		},
		{
			'name': '4. Advent Quiz',
			'abschlussseite': 'raetselende_vierteradvent2.shtml',
			'raetsel': [
			{
				'raetselname': 'Schönen vierten Advent',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin struppig', 'Man findet mich im Stall', 'Ich bin trocken', 'Niemand will wie ich sein', 'Ich bin ein Überbleibsel', 'Manche Sterne bestehen aus mir', 'Ich bin blond'],
				'loesung': ['Stroh']
			},
			{
				'raetselname': '2. Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin dunkel', 'Ich bin ein Violinkonzert von Vivaldi', 'Ich kann tief sein', 'In der Meteorologie beginne ich früher', 'Mein Name ist ein germanisches Sonderwort', 'Ich lasse vieles schlafen', 'Manchmal bin ich hart'],
				'loesung': ['Winter']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin eine Institution', 'Ich bin diplomatisch', 'Ich kann gut oder schlecht sein', 'Ich kann aus Papier sein', 'Meine Überbringer können vielgestaltig sein', '1974 ging ich an Außerirdische', 'Man findet mich in Hauptstädten'],
				'loesung': ['Botschaft']
			},
			{
				'raetselname': '3. Rätsel',
				'kategorie': 'Alles ist möglich',
				'hinweise': ['Ich bin klein', 'Tomte Tummetott', 'Ich lebe versteckt', 'Ich tue Gutes', 'In Skandinavien helfe ich dem Weihnachtsmann', 'Die Maskottchen des ZDF gehören zu meiner Art', 'Ich bin ein Diminutiv'],
				'loesung': ['Wichtel']
			}
			]
		}
	]
};
